 ...
 *  - Вся инициализация обёрнута в try/catch: при любом сбое контент принудительно
 *    показывается (revealAll), чтобы скрытое состояние никогда не осталось навсегда.
 *  - Элементы, уже видимые на момент загрузки, не прячем. Все scroll-триггеры once:true.
 */
...
/** Принудительно показать все анимируемые элементы (фолбэк при любой ошибке). */
...
export async function initAnimations(): Promise<void> {
  ...
  try {
    ...
    // Любой сбой инициализации (ошибка/404 чанка) — снимаем скрытые состояния.
    revealAll();
  }
}
